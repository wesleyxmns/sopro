...
func TestGetTopProcesses_NegativeLimit(t *testing.T) {
	procs, err := GetTopProcesses(-1)
	if err == nil {
		t.Error("Expected error when calling GetTopProcesses with negative limit, got nil")
	}
	if procs != nil {
		t.Errorf("Expected procs to be nil when limit is negative, got %v", procs)
	}
}
